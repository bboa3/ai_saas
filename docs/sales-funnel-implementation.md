...
### 2026-08-29 — Partner forms and the tenant domain as an option

MOZ ECONOMIA, SA sells through two partners: **Kaleny Holding, SU, SA** and **Curati Saúde, LDA**. Decision: the funnel is untouched (plans, Contract, billing, emails); only the signup form, the tenant domain and the installed apps differ.

- **Domain is a Select** (`mz_domain`) on Contract and MZ Signup: `.erp.mozeconomia.co.mz` (default), `.erp.curati.co.mz`, `.erp.kalenyholding.com`. Source of truth `saas/provisioning.py`: `DOMAINS`, `DOMAIN_PROFILES`, `ROUTE_BY_DOMAIN`, `domain_for()`, `domain_profile()`. `provision_tenant` builds `site_name = slug + domain`; `mz_tenant_url` follows. The desk Contract form shows the chosen domain as the slug suffix and refills the apps grid when it changes.
- **Forms**: `/registo` (default), `/registo-curati`, `/registo-kalenyholding` — one template (`www/registo/index.html`; partner folders hold a one-line child template + `build_context(context, domain)`), the domain hard-coded per form and sent with `start(…, domain)`; an unknown value falls back to the default. The page shows "Em parceria com <partner>" under the logo. Resume links (`MZ Signup.resume_url`, used by the four G1 nurture emails) return to the form the signup started on.
- **Curati profile**: sector fixed to `Saúde & Bem-Estar` (the form does not ask; whatever the browser sends is ignored) and apps `healthcare → pos_next → curati_connect` appended after the segment's, installed one by one by `_step_install_apps` (healthcare first: `curati_connect.required_apps`). `curati_connect` left `EXCLUDED_APPS`. hrms still follows the plan tier. Kaleny: domain only.
...
### 2026-08-30 — Legacy accounts: inventory before action

**Why.** Production carries what the old funnel produced (~45 sites, ~50 Customers, ~80 Leads/Opportunities) and none of it is visible to the new lifecycle: legacy sites have no provisioning row, "suspended" meant a cancelled Subscription with the site possibly still live, some sites were archived by hand. Two action plans were rejected on the same ground — *identify and categorise first; act knowing what we have* — and the site, not the CRM record, was chosen as the ground truth.

**Built.** `erpnext_mz.utils.tenant_usage.identity()` — the in-site probe's sibling: Company (name, NUIT, email, phone), the customer's System Managers (name, email, mobile, last login), the usage snapshot plus last invoice / last own-document date, DB size; read-only, never whitelisted. `ai_saas/saas/legacy_migration.py::inventory` walks `sites/` and `archived/sites/`, probes each live site, reads `maintenance_mode`, the newest backup and the public HTTP status, then matches control-site records through every key (site name → Contract/provisioning/MZ Signup; NUIT → Customer/Signup; emails → Customer, Contact, Lead, Opportunity, Signup, Contract; mobiles; normalised company name, exact or `difflib` ≥ 0.85 flagged `fuzzy`). `_pick` chooses by strength (site/NUIT/email > name > mobile > fuzzy) and lists every other strongly-matched candidate under `conflicts`; `_control_side` reads Contract, Subscription (`status`, `cancelation_date`), invoices, payments, Opportunity and Lead **of the chosen Customer only** — a contract hit through a shared email is never borrowed. `_classify` is a hint from the observed signals. Output: `tenant_inventory_<date>.xlsx` (`sites`, `control_only`, `summary`) attached to MZ SaaS Settings; the only write.

**Verified by.** 6 tests (`test_legacy_migration`: keys, fuzzy, pick/conflicts, no borrowing, classification, one File and no other write) + 1 in erpnext_mz; erp.local run: 3 live + 5 archived lab sites, matches by `email;name;site`, shared lab emails surfaced as conflicts, control_only lists the customers without a site.

**Next.** Run on production after deploy, read the `sites` sheet together, then plan Phase 2 (register provisioning rows, `Opportunity.mz_contract` so G2/G3 work without a signup, archive/close per explicit list, `suspend(notify=False)`).

### 2026-08-31 — Venda Directa: the second door into the funnel; the active legacy accounts migrated through it

**Why.** The funnel only knew `/registo`, but the inventory showed a permanent second reality: accounts negotiated in person (CCM holding, partners, earthwize, Kaleny Holding — to be billed from 2026-09-07 —, certiflor's yearly deal) and 15 active legacy customers. Created by hand they would be treated as self-service trials: countdown emails, auto-suspend at `start_date`. Decision: manual customer creation is a first-class channel, using the **same MZ Signup** — same validations, documents, provisioning and delivery email — with the signup/trial automation switched off per account.

**Built.**
- *Entry.* `MZ Signup.venda_directa` (Check; desk client script defaults it to 1 on new desk records — `/registo`'s API inserts never run it) and `MZ Signup.trial_days` (Int — the negotiation/configuration window, overriding `MZ SaaS Settings.trial_length_days`; not a promotion). Neither is in `PUBLIC_FIELDS`. Button **Criar Conta** (status Started) → `api/signup.create_account_from_desk` (whitelisted, `only_for` System/Sales Manager, never guest): required-field check with readable Portuguese labels, then the web form's own `_validate_step` per step (email/NUIT normalisation, `_resolve_city`, subdomain availability), duplicate-NUIT refusal, then the same `_create_documents`.
- *Marker.* `Contract.mz_direct` (Check "Venda Directa", `allow_on_submit`), set by the signup copy or the migration. Reads: the 6 trial notifications + 3 Pós-Contrato gained `and not doc.mz_direct`; `live_trials()` excludes direct accounts (no rule-1 auto-suspend, no probe, no trial-slot consumption); rule 2 / dunning untouched — billing is billing. G1 needed nothing: a desk signup enters the CRM only at account creation, already at `Account Created` (`_enter_crm(stage=…)`), so `Form Started` never existed — no status juggling on the signup.
- *Ledger.* `on_contract_submitted` now reports `Account Created` for unsigned contracts (desk-created accounts finally enter the ledger; the signup path's own report a moment later is a no-op).
- *Migration verbs* (`saas/legacy_migration.py`). `activate(contracts, dry_run)`: for signed legacy accounts — provisioning row inserted directly (the doctype has no hooks; nothing runs, nothing mails), Contract linked to its existing Subscription via `db_set` (`mz_linked_subscription`, `mz_billing_start = sub.start_date`, plan/tenant fields where empty, `mz_direct=1`) — the signature hooks must never run (second Subscription + invoice today), Customer primaries + commercial group, Opportunity → Activated/Converted (created at that stage when none). Zero non-cancelled Subscriptions → registered unlinked (maze, CCM); more than one → skipped for a human. `create_account(customer_name, site, plan, start_date, …)`: direct account over an **existing** site — provisioning row first so `provision_tenant` no-ops (no delivery email), Customer/Contact reused or built from the site's `identity()`, Contract submitted already signed; the normal hook creates the Subscription at `max(start_date, today)` — never back-dated; without `plan`, the engine-silent holding/partner shape. Inventory: `_is_tenant_site` accepts bare-domain sites (`erp.kalenyholding.com`), sheet gains `mz_direct`. `users=N` (2026-09-04) stamps contracted seats on the Contract before insert, so the submit hook bills `_billed_qty(N) = N-1`; without it a direct account silently bills one plan cost. The dry run prints `seats → qty x cost = total`. `prepare_account(...)` is the same verb stopping before the submit: it leaves the Contract a **draft** (already ticked Assinado) with its provisioning row registered, so a human reads the price and clicks Submit — the click that creates the Subscription and closes the Opportunity as Converted. It also reports what the Customer still lacks for a correct fiscal invoice (NUIT, email — `contact_email` is fetched at insert and never refreshed — and a billing Address, which neither direct verb creates, unlike the self-service path).

**Review fixes (same day).** (1) *Criar Conta exactly once*: the desk path now mirrors `_submit`'s discipline — row lock (`for_update`) + status flipped to Submitted and committed before any document exists, `_create_documents` wrapped so a crash marks the signup Failed (traceback kept) and only a Failed run without a Contract may retry; a second click, sequential or concurrent, is refused and the button itself only renders while Started. (2) A plan-less `create_account` (holding/partner) reported nothing — `on_contract_submitted` returns before the ledger when there is no plan — so the command itself now moves the group and reports Activated/Converted. (3) `_enter_crm`'s reuse branch resent the Dia 0 welcome even when reached with a target stage (a desk sale whose email once started a web signup) — the resend now happens only on a genuine step-1 restart. Known, accepted: `find_opportunity`'s `customer_name` fallback can touch an unrelated open Opportunity of a reused Customer (pre-existing, now also on the unsigned report).

**Verified by.** 145 tests: direct desk sale end to end (normalisation ran, `start_date = today+45`, Opportunity born at Account Created, Email Queue unchanged), guest exclusion, `activate` (links, Activated/Converted, no mail, no second Subscription, idempotent, dry-run writes nothing, unsigned refused, `account_phase == "Active"`), `create_account` (future billing date, no invoice today, no delivery email, site-already-owned refusal), direct contracts invisible to `live_trials()`.

**Ops on deploy.** `bench migrate` (signup fields, `Contract-mz_direct`, notification conditions, client scripts) → re-run `inventory` → `activate('@sheet.csv')` dry-run then live for the 15 → `create_account` for Kaleny (2026-09-07) and certiflor (renewal date). Next day: overdue reviews for believer/omp/supply/sanlo appear from `billing_monitor`'s own run.

### 2026-08-31 — Phase 3: the wind-down verbs, and the funnel reviewed whole

**Wind-down (decisions from the live inventory).** 6 still-active cancelled accounts (ayronia, sdgroup, myserv, mftechnology, alternative, bom-apetite) → registered via `activate`, then suspended by Sales through `MZ Overdue Review` (the engine archives after grace; auto_archive is on). 13 dormant → registered (`activate`; the 5 without Contracts via `create_account` with no plan), then `archive_now` — one "Conta Arquivada" email + G3. `mei` → `archive_now quiet=1`: backup kept, zero emails, campaign clock cleared. `flex` is a paying customer (6-month plan paid 10 Jul; a return invoice muddied its Subscription) → `activate CON-2026-00029`. All migration tooling is TEMPORARY: `saas/legacy_migration.py` + `tests/test_legacy_migration.py` + `erpnext_mz…tenant_usage.identity()` are deleted in one commit when the wind-down completes; nothing permanent imports them.

**Permanent piece.** `crm.find_contract(opportunity)` — the derived mirror of `find_opportunity` (signup when there is one, else party → Customer → newest cloud Contract), exposed to templates as `mz_find_contract`. G2/G3 now read the account ONLY through it (conditions dropped `doc.mz_signup`; no template reads MZ Signup for account data — G1 alone keeps it, for the resume link). Without this, every `create_account` customer — Kaleny, certiflor, CCM — was unreachable by remarketing forever. `suspend()`/`archive()` gained `notify=` (mirroring `reactivate`).

**Design review (audit of the whole funnel, same day).**
- *Correctness:* `find_opportunity` no longer matches by display name (it could mark a stranger's deal Lost at archive); one ledger report per submit (the hook defers to the signup path when an MZ Signup owns the contract); `override_doctype_class` on **Notification** — an empty rendered body (email or SMS) means *no send*, turning every `{% if %}` body-gate into a real gate; the trial countdown regained its site gate (body-gated on provisioning Active — conditions cannot query the DB); `usage_report_rows` reads `live_trials()` instead of its own join, which had already drifted (missing the `mz_direct` exclusion); billing follow-up ToDos get an owner and a per-(contract, invoice) dedup marker — one closed ToDo no longer silences follow-ups forever; the guest `_submit` takes the same `for_update` status lock as the desk path; multipay's guest endpoints are rate-limited.
- *Organization:* `saas/settings.py` (get_settings + the promised names — TRIAL_CUSTOMER_GROUP, contract/welcome template titles, booking default; ~15 cycle-breaking function-level imports gone); `saas/accounts.py` (the document factory + desk entry out of `api/signup.py`, which keeps the guest HTTP surface; `crm.py` absorbed `enter_crm`/`sync_crm`); `helpers.FunnelTestCase` (tracked-registry teardown that survives submit's commits) adopted by the four heaviest suites.
- *Parked:* **per-user pricing** — MozEconomia Cloud is priced per user but Subscriptions charge one flat plan; ~~to be designed in its own session~~ → built 2026-09-01, see the entry below.

**Verified by.** 154 tests, incl.: display-name never matches; `find_contract` for signup/Customer/Lead shapes; G3 condition + render for an Opportunity with no signup (`/reactivar` link present); an empty rendering queues nothing and the same notification sends once the site is Active again; `archive_now` = one email, Closed/Lost, clock running; `quiet` = zero emails, no clock; unregistered refused; quiet suspend sends nothing.

---

## 2026-09-01 — Per-user pricing (contracted seats)

**The decision.** MozEconomia Cloud bills per user. The model is **contracted seats**, not metered: a seat count agreed on the Contract, changed only by Sales or by the customer asking — never by counting logins. The existing plan costs became the per-user rate (Premium Mensal 2 999 MZN/utilizador/mês, Premium Anual 29 990/utilizador/ano); the plans themselves are untouched (`Fixed Rate`, cost = per-unit). **The first user is included** (review, same day): N seats bill N−1 plan costs, floor 1 (`_billed_qty`), so the entry price for the 2-seat self-service minimum stays 2 999 — and each further user adds one plan cost. ERPNext multiplies natively — `Subscription Plan Detail.qty` lands on every invoice line as quantity × the plan cost — so the whole feature is: put the right number in `qty` and keep it there.

**Single source of state.** `Contract.mz_users` (Int, `allow_on_submit`, beside the plan) is the contracted-seats source. The linked Subscription's plan-row qty mirrors `_billed_qty(mz_users)`, maintained by `_sync_subscription_seats` at the change event (`on_contract_signed`, before the signature early-returns — an already-signed contract never gets past them). Native invoice generation reads qty at generation time, so a seat change lands on the **next invoice ERPNext generates — no proration**. On a period-start day, a change saved before the daily job lands on that day's invoice; after it, on the following period's. A simultaneous plan+seats edit hits the B3 guard first and the whole save rolls back, seats included.

**The floor.** `MZ SaaS Settings.minimum_users` (default 2) is a **self-service form rule only** — enforced at `/registo` step 3, at `/activar`, and stamped by `_create_documents` when the signup carries no number. The desk passes `minimum_users=1` to `_validate_step` (Sales may contract any value ≥ 1), and `_setup_subscription`'s fallback for an empty field is **1, not the minimum** — the empty paths are desk/legacy contracts where 2 would silently double-bill an account Sales meant as one seat.

**Seats travel the funnel.** `MZ Signup.users` (default 2) → `/registo` step 3 asks it beside the plan (price copy says "por utilizador"; a total line shows `N utilizadores (1.º incluído): N−1 × custo = total`) → `_create_documents` stamps `Contract.mz_users` → `/activar` lets the customer correct it until a Subscription is linked (the same rule as the plan), and the same save that signs reads the fresh value. The pre-billing email's *Valor Estimado* is now cost × qty, with the breakdown shown when qty > 1. Clause 1 of the contract terms names the seats (`para {{ mz_users or 2 }} utilizador(es) — o primeiro está incluído no plano e os restantes são facturados por utilizador`) — the seeder never overwrites an existing Contract Template (B4), so live sites need the template updated once by hand.

**The backfill** (`patches/v1/set_contract_users_from_subscription`). Every submitted Contract with a linked Subscription gets `mz_users` = its plan-row **qty + 1** (billing-preserving under first-included) — no Subscription writes, no bill changes: flat qty-1 customers became 2-seat contracts at the same cost (exactly the new entry shape), and Sanlo's hand-set qty 5 became 6 seats. Where the recorded seats should differ (Sanlo really has 5 users → 4 paid → the bill drops one plan cost), Sales sets `mz_users` in the desk and the sync adjusts. Unsigned in-flight trials were left **empty by design**: `/activar` asks the seats question (defaulting to the floor) at the moment they convert — backfilling 1 would bypass the minimum without the customer ever seeing the question, backfilling 2 would invent a number they never chose. The patch also creates the custom field if the fixture has not synced yet (patches run before `sync_fixtures` on migrate).

**Deferred: seat-overage visibility.** Paying tenants have no usage snapshots today (`collect_usage_snapshots` sweeps trials only), so "measured users > contracted seats" needs its own follow-up: a weekly probe of signed contracts with Active sites into `MZ Tenant Usage Snapshot` (it already has `user_count` and the contract link), plus a "Contas acima dos postos contratados" section in the daily usage digest. Until then, enforcement is a Sales conversation — seats are contractual, not metered.

**Verified by.** 171 tests, incl.: qty from `mz_users` (5 seats → qty 4; 2 seats → qty 1, the entry price); empty seats bill 1 and normalise the contract to 2; a seat change on a signed contract updates the linked Subscription's qty without re-running the signature path; 0 seats on a live subscription refused; plan+seats together → B3, qty untouched; the self-service floor throws at 1 and the desk override accepts it; `_submit` maps `users` → `mz_users` and stamps the floor when missing; `/activar` sets seats and the Subscription is born with them, locked once linked; the pre-billing email renders `6 utilizadores (1.º incluído): 5 × …` with the right total; the backfill stamps linked contracts (qty + 1) and leaves unsigned ones alone.
