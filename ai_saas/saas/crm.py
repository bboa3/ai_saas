"""CRM glue shared by the usage signals (D2) and the lifecycle engine (F6):
resolving the Opportunity behind a contract, and creating the sales ToDo."""
...
def find_opportunity(contract_name):
	"""The open Opportunity behind a contract, or None (legacy contracts may have none).
	...
	if not party_name:
		return None
	open_filter = {"status": ("not in", ["Lost", "Converted", "Closed"])}
...
def set_opportunity_stage(opportunity, stage):
	"""Flip the sales stage; returns True if it actually changed."""
	if frappe.db.get_value("Opportunity", opportunity, "sales_stage") == stage:
		return False
	frappe.db.set_value("Opportunity", opportunity, "sales_stage", stage)
	return True
